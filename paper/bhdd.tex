...
nothing like those on Latin digits. This made the broader point that
writing systems with different visual structures need their own datasets.
Analogous benchmarks have since been developed for other scripts,
including Bengali~\cite{alam2018numtadb} and
Arabic~\cite{elsawy2017arabic} handwritten digits.

No public benchmark for Myanmar handwritten digits existed before BHDD.
...
and yellow sheets), each filling approximately ten pages (one per digit
class). Contributors came from Taungoo Computer University
...
clerks, programmers, and students. This range of contributors
produces natural variation in stroke thickness, pen pressure, and
letter formation across the dataset. Fig.~\ref{fig:sheets} shows
...
The filled sheets were scanned or photographed. Because many
contributors had low-quality phone cameras, we developed a mobile
application that applied adaptive thresholding and contour detection
in real time, letting users adjust parameters and retake photos until
the extracted digits were clean. The preprocessing pipeline converted
images to grayscale, binarized with adaptive thresholding, found
contours to locate each digit, cropped to the bounding box, centered
in a $28\times28$ frame, and normalized intensity. Quality assurance was a two-stage process: all 20 members of the
annotation team independently inspected every training page, and two
data engineers then performed a final verification pass over the
complete dataset. Mislabeled, illegible, and duplicate images were
removed at each stage. The training and test sets use different
contributors so that models cannot rely on memorizing individual
handwriting styles. Most contributors were based in Yangon, with
...
Digits~0 and~1 are the hardest pair: 24 misclassifications in both
directions. The only difference is whether the circle is closed or has a
small gap. Digits~0 and~8 share a round outer shape but differ
inside (plain ring vs.\ spiral), with 8 errors both ways. Digit~3 gets
mistaken for~1 seven times when its tail is faint and only the curved
head shows. Digits~5 and~8 sometimes overlap because both have looped
forms (3~errors).
...
with a macro F1 of 0.998, cutting misclassifications roughly in half
...
account for about a third of all mistakes, consistent with the
